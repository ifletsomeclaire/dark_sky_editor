...
layout(set = 3, binding = 0) uniform StarMaterial_basecolor {
    vec4 basecolor;
};

# ifdef STARMATERIAL_TEXTURE
layout(set = 3, binding = 1) uniform texture2D StarMaterial_texture;
layout(set = 3, binding = 2) uniform sampler StarMaterial_texture_sampler;
# endif
...
void main() {
    ...
    # ifdef STARMATERIAL_TEXTURE
        output_color = texture(
            sampler2D(StarMaterial_texture, StarMaterial_texture_sampler),
            v_Uv);
    # endif
