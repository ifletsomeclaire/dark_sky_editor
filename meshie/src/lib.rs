...
pub fn add_mesh(mesh: &mut Mesh, other: &Mesh) {
    match mesh.indices.as_mut().expect("mesh indices") {
        Indices::U16(_) => {}
...
mod tests {
    use super::*;
    use bevy::{math::vec2, prelude::{Mesh, shape}};

    #[test]
    fn it_works() {
        let mut mesh_one = Mesh::from(shape::Quad { size: vec2(200., 200.), flip: false});
        let mesh_two = Mesh::from(shape::Quad { size: vec2(200., 200.), flip: false});
